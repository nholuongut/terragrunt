...
func canEvaluateLocals(expression hcl.Expression, evaluatedLocals map[string]cty.Value) hcl.Diagnostics {
	...
	for _, localVar := range localVars {
		...
		case localVar.IsRelative():
			...
			detail = "This caused an impossible condition, tnis is almost certainly a bug in Terragrunt. Please open an issue at github.com/nholuongut/terragrunt with this message and the contents of your terragrunt.hcl file that caused this."
...
func (err MaxIterError) Error() string {
	return "Maximum iterations reached in attempting to evaluate locals. This is most likely a bug in Terragrunt. Please file an issue on the project: https://github.com/nholuongut/terragrunt/issues"
}
